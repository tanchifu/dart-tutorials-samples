...
handlePost(HttpRequest request) async {
  ...
  if (decoded.containsKey('myNote')) {
    print("user entered: ${decoded['myNote']}");
    sendWSRequest(request, decoded['myNote']);
  } else {
...
void saveNote(HttpRequest request, String myNote) {
  ...
  request.response..statusCode = HttpStatus.OK
    ..writeln('You have $count notes. ===> Ticker: ${myNote}')
    ..close();
}

void sendWSRequest(HttpRequest request, String symbol) {
  String url = "http://quote.yahoo.com/d/quotes.csv?s="+symbol+"&d=t&f=sl1d1t1c1ohgvj1pp2wern";
  http.get(url).then((response) {
    print(response.body);
    saveNote(request, "${response.body}\n");
  });
}
